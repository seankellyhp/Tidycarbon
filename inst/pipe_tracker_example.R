# =========================
# Example usage (quanteda stress test) --- COMPTEXT 2026, slide 12
# =========================
# carbon_init() registers the tracker as the session default, so each
# carbon_step() / carbon_run() call needs neither tracker= nor output_dir=.
# In a carbon_step() call the step is passed directly and the piped data fills
# its first argument slot.
...
dfm_big <- big_corp |>
  ...
  carbon_step(tokens_keep("^.{3,}$", valuetype = "regex")) |>
...
run <- carbon_run({
  tokens(big_corp, remove_punct = TRUE, remove_symbols = TRUE,
         remove_numbers = TRUE, remove_url = TRUE) |>
    tokens_tolower() |>
